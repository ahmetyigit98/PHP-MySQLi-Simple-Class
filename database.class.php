...
class MyDatabase
{
	...
	public function __construct($server = 'localhost', $user = '', $pass = '', $database = '', $pre = '')
	{
	...
	public function query($sql)
	{
		...
		if ($this->debug_sql == 2) {
			echo '<p>[<b>' . $duration . '</b> / ' . $this->affected_rows . '] ' . $sql . '</p>';
		}
	...
	public function query_update($table, $data, $where = '1')
	{
		...
		foreach ($data as $key => $val) {
			if (strtolower($val) == 'null') {
				...
				$v .= "UNIX_TIMESTAMP(), ";
			} elseif (preg_match("/HEX\((.+)\)/", $val) == true) {
				$q .= '`' . $key . '` = ' . $val . ', ';
			} else {
	...
	public function sql_log($msge = '')
	{
		...
		$filename = $this->log_dir . 'mysql.log';
		$fd       = fopen($filename, 'a+');
	...
	public function buildWheres($wheres = [], $start = 'where'): string
	{
